  ...
  return {
    quick: {
      ...
      payment: () => {return {
        sql: {
          select: [
            ...
            "then {CCS}' ~ '{SEP} {CAS_TEXT}p.id {CAE_TEXT} {CCE} else '' end {CCE} as lslabel",
            "{CCS}{FMS_DATE}p.paiddate {FME_DATE}{SEP}' | '{SEP} tg.groupvalue{SEP}'-'{SEP}dg.groupvalue{SEP}' | ' "+
            "{SEP}pl.curr{SEP}' | '{SEP}{CAS_TEXT}p.amount {CAE_TEXT}{CCE} as lsvalue", 
            "t.id as trans_id", "tg.groupvalue as transtype", "pl.curr"],
          from:"payment p",
        ...
        value: "{CCS}{FMS_DATE}p.paiddate {FME_DATE}{SEP}' | '{SEP} tg.groupvalue{SEP}'-'{SEP}dg.groupvalue{SEP}' | ' "+
          "{SEP}pl.curr{SEP}' | '{SEP}{CAS_TEXT}p.amount {CAE_TEXT}{CCE} "};},
      
      place: () => {return {
        sql: {
          select:["{CCS}'place'{SEP}'/'{SEP}'/'{SEP} {CAS_TEXT}p.id {CAE_TEXT}{CCE} as id",
            "{CCS}p.planumber{SEP} case when p.curr is null then '' else {CCS}' | ' {SEP}p.curr {CCE} end{CCE} as lslabel", 
      ...
      place_warehouse: () => {return {
        sql: {
          select:["{CCS}'place'{SEP}'/'{SEP}'/'{SEP} {CAS_TEXT}p.id {CAE_TEXT}{CCE} as id",
            "p.planumber as lslabel", "p.description as lsvalue"],
          from:"place p",
          inner_join:["groups pt","on",[["p.placetype","=","pt.id"],
      ...
      product_item: () => {return {
        sql: {
          select: ["{CCS}'product'{SEP}'//'{SEP} {CAS_TEXT}p.id {CAE_TEXT}{CCE} as id",
            "{CCS}p.partnumber{SEP}' | '{SEP}p.description{CCE} as lslabel",
            "p.partnumber", "p.description", "p.unit", "p.tax_id",
            "{CCS}p.unit{SEP} case when p.tax_id is null then '' else {CCS}' | '{SEP}tx.taxcode {CCE} end {CCE} as lsvalue"],
          from:"product p",
      ...
      report: (usergroupId) => {return {
        sql: {
          ...
          where:[["tg.groupvalue","=","'report'"],["and","ft.groupvalue","in",[[],"'ntr'","'xls'"]], 
      ...
      servercmd: (usergroupId) => {return {
        sql: {
          ...
          from:"ui_menu m",
          where:[["m.id","not in",[{select:["um.id"], from:"ui_audit a", 
    ...
    transitem: () => {return {
      options: {
        usergroup_filter: ["t.cruser_id","in",[{select:["id"], from:"employee", 
          where:["usergroup","=","?"]}]],
        employee_filter: ["t.cruser_id","=","?"],
        deffield_sql: {
          select:["df.id","df.fieldname","g.groupvalue as fieldtype","df.description"],
          from:"deffield df", 
          inner_join:["groups g","on",["g.id","=","df.fieldtype"]],
